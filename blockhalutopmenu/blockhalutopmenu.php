...
require(dirname(__FILE__) . '/dropdownlinks.class.php');

class blockhalutopmenu extends Module
{
    protected $_menu = '';
    protected $_extralinks = '';
    protected $_html = '';
    ...
    {
        $this->name = 'blockhalutopmenu';
        $this->tab = 'front_office_features';
        $this->version = '1.0.0';
        $this->author = 'Modified by Marius Parfene';

        $this->bootstrap = true;
        parent::__construct();

        $this->displayName = $this->l('Top menu for Halu Theme');
        $this->description = $this->l('Adds menu to the top of your website. Modified for halu theme');
    ...
    public function install($delete_params = false)
    {
        ...
        if ($delete_params) {
            if (!Configuration::updateGlobalValue('MOD_blockhalutopmenu_ITEMS', 'CAT3,CAT26') || !Configuration::updateGlobalValue('MOD_blockhalutopmenu_SEARCH', '1')) {
                return false;
            }
        }
    ...
    public function uninstall($delete_params = false)
    {
        ...
        if ($delete_params) {
            if (!Configuration::deleteByName('MOD_blockhalutopmenu_ITEMS') || !Configuration::deleteByName('MOD_blockhalutopmenu_SEARCH')) {
                return false;
            }
        }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitblockhalutopmenu')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                if (count($shops) == 1) {
                    if (is_array($items) && count($items)) {
                        $updated = Configuration::updateValue('MOD_blockhalutopmenu_ITEMS', (string)implode(',', $items), false, (int)$shop_group_id, (int)$shop_id);
                    } else {
                        $updated = Configuration::updateValue('MOD_blockhalutopmenu_ITEMS', '', false, (int)$shop_group_id, (int)$shop_id);
                    }
                }

                $updated &= Configuration::updateValue('MOD_blockhalutopmenu_SEARCH', (bool)Tools::getValue('search'), false, (int)$shop_group_id, (int)$shop_id);
            ...
            if (Tools::isSubmit('submitblockhalutopmenuLinks')) {
                ...
                foreach ($shops as $shop_id) {
                    ...
                    Configuration::updateValue('MOD_blockhalutopmenu_ITEMS', str_replace(array('LNK' . $id_linksmenutop . ',', 'LNK' . $id_linksmenutop), '', Configuration::get('MOD_blockhalutopmenu_ITEMS')));
        ...
        $this->_html = $this->_html . $this->renderList();
        return $this->_html;
    }
        ...
        if (is_array($items) && count($items)) {
            ...
            if (count($shops) > 1) {
                foreach ($shops as $key => $shop_id) {
                    $shop_group_id = Shop::getGroupFromShop($shop_id);
                    $conf .= (string)($key > 1 ? ',' : '') . Configuration::get('MOD_blockhalutopmenu_ITEMS', null, $shop_group_id, $shop_id);
                }
            } else {
                $shop_id = (int)$shops[0];
                $shop_group_id = Shop::getGroupFromShop($shop_id);
                $conf = Configuration::get('MOD_blockhalutopmenu_ITEMS', null, $shop_group_id, $shop_id);
            }
    ...
    protected function makeMenuExtralinks()
    {
        $menu_items = $this->getMenuItems();
        $id_lang = (int)$this->context->language->id;
        $id_shop = (int)Shop::getContextShopID();
                ...
                case 'LNK':
                    ...
                    if (is_array($link)) {
                        if (count($link)) {
                            ...
                            $this->_extralinks .= '<li><a href="' . Tools::HtmlEntitiesUTF8($link[0]['link']) . '"' . (($link[0]['new_window']) ? ' onclick="return !window.open(this.href);"' : '') . ' title="' . Tools::safeOutput($link[0]['label']) . '" data-depth="0">' . Tools::safeOutput($link[0]['label']) . '</a></li>' . PHP_EOL;
                        }
                    }
                break;
            }
    ...
    protected function generateCategoriesMenu($categories, $is_children = 0)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            $html .= '<li class="category" id="category-'.(int)$category['id_category'].'">';
            /*$html .= '<a href="' . $link . '" title="' . $category['name'] . '">' . $category['name'] . '</a>';*/
            $html .= '<a href="' . $link . '" class="dropdown-item" data-depth="0">' . $category['name'] . '</a><span class="hidden-md-up"><span data-target="#top_sub_menu_54321" data-toggle="collapse" class="navbar-toggler collapse-icons">  <i class="material-icons add">&#xE313;</i><i class="material-icons remove">&#xE316;</i></span></span>';

            if (isset($category['children']) && !empty($category['children'])) {
                $html .= '<div class="popover sub-menu js-sub-menu collapse" id="top_sub_menu_54321" style=""><ul class="top-menu" data-depth="1">';
                $html .= $this->generateCategoriesMenuChild($category['children'], 1);
    ...
    /*change childern cat*/
    protected function generateCategoriesMenuChild($categories, $is_children = 0, $datadepth = 1) {
        $html = '';
        ...
        foreach ($categories as $key => $category) {
            ...
            /*$html .= '<div class="halu-menu-children">'.'<span' . (($this->page_name == 'category'
                    && (int)Tools::getValue('id_category') == (int)$category['id_category']) ? ' class="sfHoverForce"' : '') . '>';
            $html .= '<a href="' . $link . '" title="' . $category['name'] . '">' . $category['name'] . '</a>';*/
            $html .= '<li class="category" id="category-'.(int)$category['id_category'].'">';
            $html .= '<a href="' . $link . '" class="dropdown-item" data-depth="'.$datadepth.'">' . $category['name'] . '</a>';

            /*Peste al treilea nivel categorii*/
            if (isset($category['children']) && !empty($category['children'])) {
                $html .= '<ul class="top-menu" data-depth="2">';
                $html .= $this->generateCategoriesMenuChild($category['children'], 1, 2);
                $html .= '</ul>';
            }
    ...
    public function hookdisplayHeader()
    {
        ...
        $this->context->controller->addJS($this->_path . 'js/blockhalutopmenu.js');
        $this->context->controller->addCSS($this->_path . 'css/superfish-modified.css');
        $this->context->controller->addCSS($this->_path . 'css/blockhalutopmenu.css');
    }
    ...
    public function hookDisplayTop($param)
    {
        ...
        if (!$this->isCached('blockhalutopmenu.tpl', $this->getCacheId())) {
            if (Tools::isEmpty($this->_menu)) {
                $this->makeMenu();
            }
            if (Tools::isEmpty($this->_extralinks)) {
                $this->makeMenuExtralinks();
            }

            $shop_id = (int)$this->context->shop->id;
            $shop_group_id = Shop::getGroupFromShop($shop_id);

            $this->smarty->assign('MENU_SEARCH', Configuration::get('MOD_blockhalutopmenu_SEARCH', null, $shop_group_id, $shop_id));
            $this->smarty->assign('MENU', $this->_menu);
            $this->smarty->assign('MENUEXTRALINKS', $this->_extralinks);
            $this->smarty->assign('this_path', $this->_path);
        }

        $html = $this->display(__FILE__, 'blockhalutopmenu.tpl', $this->getCacheId());
        return $html;
    }
    ...
    {
        $this->_clearCache('blockhalutopmenu.tpl');
    }
    ...
    public function renderAddForm()
    {
        $fields_form = array(
            'form' => array(
                ...
                'submit' => array(
                    'name' => 'submitblockhalutopmenuLinks',
                    'title' => $this->l('Add')
                )
            ),
        );
    ...
    public function getConfigFieldsValues()
    {
        ...
        foreach ($shops as $shop_id) {
            $shop_group_id = Shop::getGroupFromShop($shop_id);
            $is_search_on &= (bool)Configuration::get('MOD_blockhalutopmenu_SEARCH', null, $shop_group_id, $shop_id);
        }
