...
<div class="slideshow-container">
    {foreach from=$product.images item=image}
      <div class="mySlides fadex product-cover">
        <img src="{$image.bySize.large_default.url}" class="img-fluid" alt="">
        <div class="layer hidden-sm-down" data-toggle="modal" data-target="#product-modal">
          <i class="material-icons zoom-in">search</i>
        </div>
      </div>
    {/foreach}
      <a class="prev" onclick="plusSlides(-1)"><img src="{$urls.img_url}right.svg" /></a>
      <a class="next" onclick="plusSlides(1)"><img src="{$urls.img_url}right.svg" /></a>
</div>
<div style="text-align:center;margin-top: -15px;">
  {foreach from=$product.images item=image}
    <span class="dotx" onclick="currentSlide({$image.position})"></span> 
  {/foreach}
</div>
